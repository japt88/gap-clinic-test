...
namespace ClinicAppointmentApp.Repositories
{
    public class AppointmentRepository : GenericRepository<Appointment>, IAppointmentRepository
    {
        ...
        {
            return _context.Appointments.Where(x => x.Date == date.Date && x.StartTime == startTime && x.SpecializationId == specializationId);
        }

        public IEnumerable<Appointment> GetAppointmentsByDate(DateTime date)
        {
            return _context.Appointments.Where(x => x.Date == date.Date);
        }

        public IEnumerable<Appointment> GetAppointmentsByDateAndTime(DateTime date, TimeSpan startTime)
        {
            return _context.Appointments.Where(x => x.Date == date.Date && x.StartTime == startTime);
        }
